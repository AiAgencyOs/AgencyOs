...
| **G-114** | ~~An accepted quotation is not required to raise a project~~ → **Whether a project has one is auditable but not visible** | `proposal_id` is nullable with a partial index on the non-null case; conversion carries the link when it exists and never demands it. **ADM-72 resolved the conflict by ruling on *creation* and leaving *start* exactly as ADM-13 granted it** — the two documents addressed different moments, which was the real gap | A surface showing whether a project has an accepted quotation | C | P2 | G-011, G-017 | none | **ADM-72 — GRANTED 2026-08-14, option B.** Eight of nine requirements already met with no code. Auditability **verified, not assumed**: `projects.projects` carries the G-093 trigger, yielding `project.created` on insert and `project.updated` for a later link. The ninth is unmet — **nothing displays it** | 12 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 97 |
| B — partial | 2 |
| C — missing | 16 |
| D — incorrect | 0 |
