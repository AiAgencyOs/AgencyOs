...
| **G-012** | Follow-up automation | `crm.leads.next_follow_up_at` exists, is indexed, is set by hand, and **nothing reads it**. No sender, no scheduler, no handler | Consent, then a cadence | C | P2 | G-040, G-014 | none | **`ADM-70` GRANTED 2026-08-14** — consent is required, **channel-specific**, and suppression must be enforced by the communication system rather than the UI. **`ADM-69` still unanswered**: triggers and timing. ADM-70 decides *whether* a message may go; ADM-69 decides *when*. The transactional-versus-sales portion is **REQUIRES CLARIFICATION** — no AgencyOS document uses the words *transactional*, *marketing* or *promotional* | 5 |
...
**78 Admin decisions** have been raised across these gaps; **71 are granted, 7
