...
| **G-126** | Seven activity kinds are all audited as a note | `audit.record_row_change` names every `lead_activities` row `lead.note_added` whatever its kind, so an assignment, a logged call, an inbound message and an agent run are all filed as **notes** — in `audit.audit_log`, the record that exists to be trusted. G-010 fixed it for the six kinds it added and left the seven that came before, because correcting those changes what the log says about behaviour nobody is touching | Each kind audited as itself | C | P3 | — | `tests/sales-activities.test.ts` pins the current behaviour so a change is deliberate | **None** — but it wants its own change and review rather than riding along with a feature | 9 |
