...
| **G-117** | `projects.lead_id` is the delivery lead and reads like a CRM lead | A foreign key to **`core.users`** — a member of staff. Every other `lead_id` in the system is a `crm.leads` reference | A name that says which it is | C | P2 | — | `tests/requirement-chain.test.ts` asserts neither new function joins on it | No — a rename is one migration, filed rather than folded into G-020 because renaming a column in the same change that adds two tables makes the diff about the rename | 12 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 85 |
| B — partial | 5 |
| C — missing | 14 |
| D — incorrect | 1 |
