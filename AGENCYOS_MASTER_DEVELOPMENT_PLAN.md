...
| **G-110** | The agent does not yet raise approvals in the internal group — **write direction built** | **Half built, and both halves of why are recorded.** Raising an internal-audience approval emits `approval.requested`; the catalog routes it to `crm:announceApproval` and the runner drives it into the group through the existing outbound path — so what needs deciding reaches a phone rather than only a web page. Each request carries a **six-character reference** over an alphabet with the characters people misread on a phone removed, unique per organization *forever* rather than among pending requests, because a recycled code makes a late reply land on a different decision. A **client-audience request is not announced**: §5.1 calls the group an approval channel, not a chat log. Announcing is idempotent on the request id, so a re-dispatched event, a retried job and a second raise collapse onto one message. **Not built:** nothing settles an approval from a reply — `decide_approval` refuses without `auth.uid()` (§29, the rule that stops an automation approving its own work), and a sender is an unverified, spoofable phone number that `core.users` has no column for. **ADM-74.** A correlation table and matcher were written and **removed before commit** on finding they had no possible producer — that is **G-115** | The agent raises there and an answer settles the request | B | P1 | G-109, G-040, G-014 | `tests/approval-announcements.test.ts` (19), `scripts/verify-approval-announcements.mjs` (16 live, watched failing first) | **ADM-11 — answered**; **ADM-74 — open** | 10 |
...
**74 Admin decisions** have been raised across these gaps; **64 are granted, 10
