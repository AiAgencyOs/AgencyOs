...
| **G-125** | ~~Two registered agents cannot run~~ → **The seeded registry does not match the approved roster** | `seed.sql` enables three agents; one is reachable; `ARCHITECTURE.md` §6.2 names a registry file that does not exist. **And `proposal_drafter`'s seeded description claims it drafts *pricing*, which business rules 08 §5.1 forbids absolutely** — approval does not cure it | Registry consistent with ADM-82, **proven by tests** | C | P2 | — | `agent-registry` (17), `agent-tools` (11), `agent-handoffs` (12) · `check-record` §13–§16 | **F1–F3 landed.** Registry, tool boundary and handoffs exist; **8 of 11 conditions met**. **ADM-82/83/84 GRANTED** — and **approval does not close it**. The owner fixed **eleven conditions the repository must prove**, including that forbidden authority states are *unrepresentable* rather than refused | 25 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 100 |
| B — partial | 2 |
| C — missing | 15 |
| D — incorrect | 0 |
