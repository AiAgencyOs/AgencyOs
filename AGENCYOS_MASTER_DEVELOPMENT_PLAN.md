...
| **G-136** | Consent for a project group is unmodelled | The consent model ADM-70 granted is **per contact per channel**. A `project_group` conversation has **no contact at all** — it is created with a `project_id` and nothing else, because a WhatsApp group is not a person. Applying a per-contact rule to it is a category error in either direction: the first draft of G-135's guard refused **every** group send and **CI caught it** breaking the group messaging G-014 and G-109 deliberately built, while pretending the group "consented" would invent a record nobody made. **So a project group is currently gated by no consent record**, and that is stated plainly rather than hidden. It is narrow — project groups only, never direct client messaging, never the internal group | A decision about what membership of a project group means | C | P3 | — | `tests/communication-consent.test.ts` pins the guard to `direct`, so widening it is deliberate | **Requires a business decision nobody has taken** | 9 |
...
| **G-012** | Follow-up automation | `crm.leads.next_follow_up_at` exists, is indexed, is set by hand, and **nothing reads it**. No sender, no scheduler, no handler | Four rhythms, eight situations, consent first | C | P2 | G-040, G-014 | none | **`ADM-69` and `ADM-70` both GRANTED 2026-08-14.** Seven of eight situations fully decided; **payment follow-up DEFERRED** behind `ADM-81`, the transactional communication policy. Implementation **not authorised** — the gate is still active | 5 |
...
| **G-113** | ~~The onboarding checklist is not configurable by project type~~ → **The baseline is hardcoded, so an Admin cannot configure it** | The 17 items are a literal `VALUES` list inside `projects.seed_onboarding`, so changing the baseline is a **migration, not an Admin action**. `not_applicable` lets a project opt *out* of an item; nothing lets one opt *in* to an extra | An Admin-configurable baseline | C | P3 | G-017 | none | **ADM-73 — GRANTED 2026-08-14, and it reframes the gap.** `ADM-80` now records the design review the decision demanded before implementation. §6 asked for configurability by *project type*; the owner rules type is the **wrong axis** — the agency sells arbitrary combinations and an enum would limit what can be sold. **Inspection found no `project_type` field anywhere**, so that flexibility is already the status quo. Proposal recorded for review; deliberately not built | 12 |
...
**83 Admin decisions** have been raised across these gaps; **80 are granted, 3
