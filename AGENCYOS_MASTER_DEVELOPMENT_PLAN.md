...
| **G-113** | The onboarding checklist is not configurable by project type | Every project gets the same seventeen items | Document 10 §6's closing sentence: *"The checklist should be configurable by project type."* Softened but not closed by `not_applicable`, which lets a project with no design references say so honestly rather than leaving an item pending for ever or ticking a lie — a workaround for a missing template, not the template | C | P3 | G-017 | `tests/onboarding.test.ts` pins the fixed list | **ADM-73 — open**: which project types does the agency actually distinguish? A template table is easy; inventing the type list is what would be wrong | 7 |
