...
| **G-113** | The onboarding checklist is not configurable by project type | Every project gets the same seventeen items. `not_applicable` softens it — a project with no design references can say so honestly rather than leave an item pending for ever or tick a lie — but that is a workaround for a missing template, not the template | Templates keyed by a real project-type vocabulary | C | P3 | G-017 | none | **ADM-73 — REQUIRES CLARIFICATION 2026-08-14.** Option A preferred; approval withheld until the owner supplies the actual types. **Nothing invented, no example types, no populated table, no empty system called complete** | 12 |
...
**77 Admin decisions** have been raised across these gaps; **68 are granted, 9
