...
**Baseline commit:** `249a7cd` on `main`
...
scan and build, all green on `249a7cd`.
...
**The queue is no longer defect-driven.** What remains is **19 missing
...
Beyond those, 19 missing features are each waiting on a business rule that has
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 55 |
| B — partial | 8 |
| C — missing | 19 |
| D — incorrect | 3 |
