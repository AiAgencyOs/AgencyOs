...
| **G-124** | Eleven references still name a renumbered decision | The 2026-08-14 renumbering moved the six open questions to `ADM-71`–`ADM-76` and renumbered **the plan and `roadmap.json`**. It did not renumber the eleven references living elsewhere — in migrations, services, tests and verify scripts — each of which now names a **merge approval** where it means a **question**. `check-record` could not have caught it: it compares the plan against `roadmap.json` and reads nothing else | Every reference naming the decision it means | C | P2 | — | none | **Mechanical, and held only because the references live in product files** and the gate bars product changes. A proposed check would catch the whole class: a decision titled *"Merge approval for PR…"* is bookkeeping and must never be cited outside the record | 21 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 94 |
| B — partial | 2 |
| C — missing | 17 |
| D — incorrect | 0 |
