...
| ID | Gap | Current | Required | Class | Risk | Depends | Tests | Admin decision | Phase |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **G-010** | ~~Sales lifecycle vocabulary mismatch~~ → **The six sales activities ADM-10 names cannot be recorded** | **Audited 2026-08-14 against granted ADM-10, and it splits in two.** §7 as written: *"The pipeline stays four stages: `discovery → proposal → negotiation → won`, with `lost` off any of them."* **The stage half is satisfied exactly** — the four stages are the four, and `lost` is the off-ramp §7 names separately, not a fifth stage. `OPPORTUNITY_STAGES` and the `sales.opportunities` CHECK both carry those five values, which is four stages plus the off-ramp. The suspicion that *four* might conflict with a five-value enum is answered: §7 spells the four out by name. **The activity half is not.** §7 requires contacted, sample sent, demo sent, offer sent, follow-up and advance requested to be timestamped activities on the lead; `crm.lead_activities.kind` is CHECK-constrained to seven values and **none of the six is among them**. `sample_sent`, `demo_sent`, `offer_sent` and `advance_requested` appear nowhere in the tree at all; the only matches for the other two are unrelated — `contacted` is prose in a finance comment, `follow_up` is the inert `next_follow_up_at` column. **The six states the decision moved out of the pipeline were never given the home it moved them to.** | The six recordable as activities | C | P1 | — | `tests/workflow-regression.test.ts` pins the stage set; nothing covers the activity half because it does not exist | **None — ADM-10 already answers this.** Reclassified E → C: an implementation gap, not a decision gap | 11 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 96 |
| B — partial | 2 |
| C — missing | 16 |
| D — incorrect | 0 |
