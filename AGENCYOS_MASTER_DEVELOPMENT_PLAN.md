...
| **G-120** | Leads converted before G-086 keep a null qualification date | `leads_qualified_at_set` is `NOT VALID`, so historic rows are not refused | A decision on whether to backfill | C | P3 | G-086 | None | **ADM-67 — open**: backfill, and with what date? The conversion date is the only defensible candidate and is still a guess; the alternative is a visible hole rather than invented history | 5 |
| **G-121** | Six decision ids carry two meanings each | `ADM-62`–`ADM-67` are each defined **twice** in `roadmap.json`: once granted on 2026-08-13 as the merge approval for PRs #71–#76, once open as a business question raised later the same day. Both sets are real and both are wanted — what is not wanted is that an answer addressed to *"ADM-64"* cannot be resolved to one of them (the granted one approved PR #73; the open one asks which project types the agency distinguishes). `check-record.mjs` compared `adminDecisions.length` against a number in the plan and both read 72 — **agreement is not correctness when both sides count the same duplicates.** 72 records over 66 ids | One id, one meaning | C | P3 | — | `scripts/check-record.mjs` §0, proved red against three shapes | **Detection added; the renumbering itself is the owner's** — it changes the identifiers they are about to answer the gate by | 21 |
...
| **G-116** | A group message is attributed to an audience, not a person | The sender's number is on the message metadata; `author_type` is the group's audience and `author_id` is null | Attribution to the person who actually sent it | C | P3 | G-115 | `tests/group-ingest.test.ts` pins the deliberate null | **ADM-66 — open**: is a phone match good enough, or does it need the verified link ADM-65 asks for? A wrong match puts words in a named client's mouth in a transcript they can see | 10 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 93 |
| B — partial | 2 |
| C — missing | 15 |
| D — incorrect | 0 |
...
**72 Admin decisions** have been raised across these gaps; **63 are granted, 9
