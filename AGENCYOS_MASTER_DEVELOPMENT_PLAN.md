...
| **G-012** | Follow-up automation — **BLOCKED_BY_BUSINESS_DECISION** | `crm.leads.next_follow_up_at` exists, is indexed, is set by hand, and nothing reads it | Two decisions, neither raised as an ADM id per instruction: **which situations trigger a follow-up and with what timing** — ADM-11 authorises automatic sending but does not say when or to whom — and **whether a consent model exists at all**. `crm.contacts` has no opt-in, opt-out, consent or unsubscribe column and no migration handles consent, so there is no communication permission to respect. Sending automated messages to contacts with no recorded consent is a larger exposure than the one ADM-11 accepted | C | P2 | G-040, G-014 | None | **Yes — two, and deliberately not invented** | 11 |
...
**78 Admin decisions** have been raised across these gaps; **70 are granted, 8
