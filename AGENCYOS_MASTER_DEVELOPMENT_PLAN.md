...
| **G-114** | An accepted quotation is not required to raise a project | The link is carried when it exists and never demanded | Document 10 §2 says a project *"should not be created"* without commercial acceptance. Not enforced, for two recorded reasons: ADM-13's three start conditions deliberately exclude it — an advance is not paid without acceptance, so the commercial reality is covered at *start* rather than at *creation* — and every project predating quotations would be stranded by a hard gate | C | P2 | G-011, G-017 | `tests/onboarding.test.ts` asserts the deliberate behaviour | **ADM-72 — open**: does it become a fourth start condition? It decides whether the agency may do work it has not written a price for | 7 |
...
**77 Admin decisions** have been raised across these gaps; **67 are granted, 10
