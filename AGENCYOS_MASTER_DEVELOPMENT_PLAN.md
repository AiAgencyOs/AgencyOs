...
| **G-116** | A group message is attributed to an audience, not a person | The sender's number is on the message metadata; `author_type` is the group's audience and `author_id` is null | Attribution to the person who actually sent it | C | P3 | G-115 | `tests/group-ingest.test.ts` pins the deliberate null | **ADM-75 — open**: is a phone match good enough, or does it need the verified link ADM-74 asks for? A wrong match puts words in a named client's mouth in a transcript they can see | 10 |
...
**74 Admin decisions** have been raised across these gaps; **65 are granted, 9
