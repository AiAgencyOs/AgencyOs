...
| **G-123** | The current Meta Embedded Signup requirements are unknown | Nothing in this repository establishes that the agency is **eligible** for Embedded Signup, which **permissions** the flow needs, whether **business verification** is a prerequisite, or what the current integration flow actually is | Those four facts, read from Meta's current documentation | C | P2 | — | none, **and none is possible** — no CI or database test can establish provider eligibility | **EXTERNAL VERIFICATION REQUIRED.** Not a business decision and not a coding task; the facts live at Meta. Blocks G-091's *design*, not its decision — ADM-57 is granted. Separate from G-110's UNVERIFIED state, which concerns delivery and group behaviour rather than onboarding | 12 |
