...
**Baseline commit:** `b5f5bbc` on `fix/audit-in-the-transaction` (one commit ahead of `main`)
...
scan and build, all green on `b5f5bbc`.

**One pull request is open — #40 (G-079)**, which moves the four audit writes
that sit beside a Postgres function inside that function's transaction. It is
green on CI and awaiting Admin merge approval. Nothing is being implemented
behind it.
...
**43 Admin decisions** have been raised across these gaps; **14 are granted, 29
remain open** — of which exactly one is a merge gate (ADM-44, PR #40). ADM-38 was never issued — the numbering skips it — and is
...
**ADM-44 — Merge approval for PR #40 (G-079).** The four audit writes that sit
beside a Postgres function now append from inside that function's transaction,
so a payment can no longer commit with no history — permanently, since
`audit.audit_log` is append-only by trigger. Green on CI. The remaining twelve
call sites are split out as G-093 rather than folded in.
...
| 2026-08-12 | (this change) | **The record reconciled against the repository, and the drift recorded rather than silently repaired.** `roadmap.json` described commit `5b6cbbf`; it now describes `b5f5bbc`, with measured metrics (36 migrations, 895 tests in 168 suites across 32 files) rather than remembered ones. D16–D22 were carried as open after all seven had merged; ADM-36 and ADM-37 as pending after the merges they asked for had happened. Totals are computed from the gap records: **83 gaps**, A36/B9/C29/D5/E4. The failure that allowed all of this is itself recorded as **G-094**. **ADM-44** is raised for PR #40, which had no merge-approval decision against it, breaking the one-per-PR convention. ADM-27–ADM-37 existed only in the JSON and are now listed in §5; **ADM-38 was never issued**, and the hole is noted so it is not later read as a lost decision. No source file, migration or test was touched. |
