...
| **G-128** | Recursive handoff depth is unbounded | ADM-83 bounds **where** a handoff may go — the receiver must be in the sender's targets — and not **how far** a chain runs. `A → B → A → B` is legal under every recorded rule. `max_cost_minor` bounds the damage in **money, not time**, and only after the budget is spent; a loop inside one correlation chain reads as progress | A depth bound, per correlation chain | C | P2 | G-125 | none yet | **Engineering decision recorded**: fail-safe is stop-and-escalate, scoped per correlation chain. **The number belongs with the implementation**, where it can be tested against a real chain rather than guessed | 25 |
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 99 |
| B — partial | 2 |
| C — missing | 16 |
| D — incorrect | 0 |
