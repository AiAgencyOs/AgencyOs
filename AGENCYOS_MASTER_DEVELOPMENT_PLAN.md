...
| **G-110** | The agent raises approvals in the internal group — **write direction built, and one line of it now contradicts the decision** | Raising an internal-audience approval emits `approval.requested`; the catalog routes it to `crm:announceApproval` and the runner drives it into the group. Each request carries a six-character reference over an alphabet with the characters people misread on a phone removed, unique per organization **forever** — because a recycled code makes a late reply settle the wrong thing. **ADM-74 granted option A: the reply is advisory and settles nothing.** That makes the announcement's closing line — `Reply quoting <reference>.` — **false**: nothing reads that reply and nothing ever will, so an approver who follows the instruction believes they approved something they did not. Found while recording the decision; **not changed**, because the gate bars product changes | The wording to direct the approver to AgencyOS | B | P1 | G-109, G-040, G-014 — all closed | `tests/approval-announcements.test.ts` — the wording is corrected, and a new test asserts the message does **not** invite a reply | **ADM-74 — GRANTED 2026-08-14.** Externally **UNVERIFIED**: no real WhatsApp Official Business Account has ever been used | 12 |
