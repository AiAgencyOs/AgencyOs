...
| **G-125** | Two registered agents cannot run, and the registry they come from does not exist | `supabase/seed.sql` registers **three** enabled agents — `lead_qualifier` (L2), `requirement_collector` (L1), `proposal_drafter` (L1) — and CI applies it after `db reset`, so this is the deployed state. But the runner carries a single `AGENT_KEY`, `requirement_collector`. **Two are registered, enabled and unreachable**, and `lead_qualifier` is L2, which `mayAgentRun` refuses outright. `ARCHITECTURE.md` §6.2 names a registry at `src/modules/agents/registry.ts` — **that file does not exist** — and specifies a fourth agent, `ui_generator`, which is not even seeded | An Admin reading `ai.agents` should see what can actually run | C | P2 | — | none | **No decision covers it.** Building, disabling or removing them is the agent-roster decision nobody has taken | 25 |
...
**79 Admin decisions** have been raised across these gaps; **72 are granted, 7
