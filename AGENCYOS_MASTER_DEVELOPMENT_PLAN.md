...
| **G-065** | **D10 — state transitions write with an id-only predicate** | `setLeadStatus`, `setOpportunityStage` and `setProjectStatus` each read the current state, decide against their TS transition map, then write matching on id alone. A concurrent transition is silently overwritten — a won deal can be recorded as lost | The compare-and-swap the finance functions already use: `.eq('status', from)` | D | P1 | — | `tests/workflow-regression.test.ts` pins the maps only | No — same rule | 5 |
...
| **G-068** | **D13 — reopening a disqualified lead keeps its reason** | `setLeadStatus` writes `disqualified_reason` only when disqualifying and never clears it, so a reopened lead still renders the reason it was rejected for | Clear it on the way out | D | P3 | — | None | No | 5 |
| **G-069** | **D14 — the lead status form offers a value the service always refuses** | The page hands `LEAD_TRANSITIONS[status]` straight to the control, so a qualified lead defaults to `converted` — which `setLeadStatus` refuses, because conversion happens through the sales path | Filter the options to what the service accepts | D | P3 | — | None | No | 5 |
