...
| **G-115** | Inbound group messages have no ingest path | `crm.ingest_whatsapp_message` resolves an organization by phone number id and then unconditionally creates a contact, a lead and a **`direct`** conversation. It has no notion of `kind` at all | A message in a project or internal group is filed against that group | C | P2 | G-109 | None | No — G-109 built groups as an outbound channel and this is the missing inbound half | 10 |
...
Counted from the 104 gap records in `docs/roadmap/roadmap.json`, which is the
...
| Class | Count |
| --- | --- |
| A — already implemented or fixed | 83 |
| B — partial | 5 |
| C — missing | 14 |
| D — incorrect | 1 |
...
**70 Admin decisions** have been raised across these gaps; **63 are granted, 7
