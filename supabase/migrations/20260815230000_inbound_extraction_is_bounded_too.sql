-- ═══════════════════════════════════════════════════════════════════════════
-- Inbound ingestion bounds the extraction it queues, exactly as the runner reads.
--
-- The companion to bounding the extraction transcript. `crm.ingest_whatsapp_message`
-- — the primary path, every inbound message — queues a `requirement.extract`
-- job keyed on the FULL conversation message count. The runner reads at most
-- MAX_EXTRACTION_MESSAGES (1000, the PostgREST max_rows cap) and writes the
-- version at that bounded length, so above the cap the two disagreed: every new
-- inbound message queued a fresh extraction that no-op'd against the version
-- already written at 1000, and the requirement scope froze. The dedupe key here
-- now bounds the count the same way (`least(v_count, 1000)`), so past the cap it
-- stops changing and the churn stops. This is the one live SQL enqueue of
-- extraction; the TypeScript path (requestExtraction, the human click) is bounded
-- in the same commit. `create or replace`, reproducing the current function with
