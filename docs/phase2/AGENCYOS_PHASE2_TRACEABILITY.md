...
| Context-first: never re-ask what Phase 1 confirmed | PM §4.1, Master §1 | **EXISTS** (G-252, G-276) | `onboarding-context.ts`, `resolveProjectContext`, shown in the onboarding section | Built as a JOIN over `ai.handoffs.unresolved` and `crm.qualification_coverage`. Recorded as EXISTS while **`resolveProjectContext` was dead** — one occurrence in the repository, its own definition — until G-276. Reconfirmation raised as **ADM-107** |
| Staged client requests + follow-up | PM §4.2, §6 PM-05 | **PARTIAL** (G-266, G-276) | `projects.outstanding_client_requests` + `resolveProjectContext`, shown in the onboarding section | The **cadence** is **ADM-109**, unanswered: no policy covers onboarding, and borrowing another rhythm would be inventing one. The *request* half is now reachable — G-266's record deferred the surface to ADM-109 and **that was wrong**: showing the next question needs no cadence |
...
| Default internal team members, configurable | Master §6, PM §8, P2-05 | **EXISTS** (G-267) | `projects.group_team_defaults` + `add_team_default` / `set_team_default_active` / `remove_team_default`, on `/settings` | Recorded as EXISTS on G-253 when it was not: the table had a SELECT policy, no write policy and **no door**, so the *configurable* half of this row was missing and every card shipped with no agency team on it. Changes apply to groups requested from then on — a card copies the roster rather than referencing it |
...
| Admin verification gate | Finance §4.7, Master §5.8 | **EXISTS** (G-270, G-271) | `finance.verify_payment` + the Confirm action on the invoice page; `verify_payment_submission` now takes CONFIRM / REJECT / MISMATCH | Recorded as EXISTS when **nothing could call it** — no invoice could become paid from G-007 until G-270. The **claim** layer (Doc 15 §11/§12) is still unreachable on both sides: **G-272**, raised not built, because a verify surface alone would be an always-empty list |
...
| **Billing mode GST / NON_GST** | Finance §4.1–§4.3, Master §5.6 | **EXISTS** (G-255, G-275) | `finance.billing_profiles`, `confirm_billing_mode`, `record_billing_details`, `gstin.ts`, and the billing section on the project page | **This row was wrong when written** — see the correction below. And EXISTS was premature again: until G-275 **neither door had a caller**, so G-259's gate would have blocked every invoice with a message nobody could act on |
...
| Phase 8 conditional finance triggers | Finance §10 | **MISSING** | — | Phase 8 does not exist. `maintenance.free_invoice_issued` (G-269) is the signal §9 says should reach it, emitted with no subscriber. **Phase 8 is named in the Finance document and in none of the other three** |
...
| ₹0 free-maintenance invoice, no verification | Finance §9 | **EXISTS** (G-269) | `projects.maintenance_plans.entitlement`, `finance.issue_free_maintenance_invoice`, on the project page beside the payment ladder | *No verification* is **not** a bypass of ADM-04: `paid` follows `net_verified >= total`, which is already true at zero. **Sharing it is not built** — §9 asks for email and the project WhatsApp group (BLK-007, BLK-003). Phase 7 *completion* is recorded nowhere, so a person calls the door |
...
| Validation before `ProjectPlanReady` | Planning §18 | **EXISTS** (G-265) | `projects.validate_project_plan`, gating activation | Six of §18's ten rules are structural and deliberately not re-checked. Extensively unit-tested against the migration's own text (`tests/a-plan-that-validates-itself.test.ts`) but had never been driven against a real Postgres. Driven live 2026-09-26 on a scratch database: all four non-structural rules (coverage, N/A-without-reason, phase-without-milestone, unmapped finance gate) fire individually and in combination, clear correctly once fixed, and the plan validates once all four hold; the `plan_deliverables_has_approved_source` CHECK (one of the six structural rules) independently refuses a sourceless deliverable. No regression found; PHASE2_AUDIT_2026-09-21's "not independently re-run" note is now stale |
