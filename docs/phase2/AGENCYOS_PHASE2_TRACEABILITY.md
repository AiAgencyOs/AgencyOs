...
| Admin verification gate | Finance §4.7, Master §5.8 | **EXISTS** (G-270) | `finance.verify_payment` + `verifyPayment` + the Confirm action on the invoice page; records verifier, moment, evidence | Recorded as EXISTS when **nothing could call it** — no invoice could become paid from G-007 until G-270. Decision vocabulary is still confirm/reject; **MISMATCH (§6, §16) is missing**, and `verify_payment_submission` still has no caller |
