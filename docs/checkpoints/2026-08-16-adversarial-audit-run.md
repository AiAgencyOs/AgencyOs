...
- **Two known CI flakes** — **Resolved (this change).** Both were HARNESS bugs, not product races. `verify-requirement-proposal` §Q: the runner drains `milestone.unlock` before extraction and early-returns on claim (a correct product behaviour), and `core.claim_jobs` is org-agnostic, so a stray cross-script `milestone.unlock` job diverted a runner out of extraction — the probe now parks every queued job except its own two before firing (mechanism red-proved: a stray unlock makes a tick answer `kind: milestone.unlock`). `verify-milestone-invoicing` §7e: the test computed its expected blocking invoice from an *unordered* PostgREST read while the helper returns `order by number limit 1` — the harness now mirrors that ordering. Neither assertion weakened; §Q green against the app, billing green 3× consecutively. Also pinned: the extraction dedupe SQL cap (`least(v_count, 1000)`) now tested to equal `MAX_EXTRACTION_MESSAGES` (`tests/crm-ingest.test.ts`).
