...
const ITEMS: readonly Omit<ConfigItem, 'present'>[] = [
  ...
  { key: 'GEMINI_BASE_URL', area: 'AI provider', secret: false, requiredInProduction: false, note: 'Test-only override; production must NOT point it at an external host.' },
  { key: 'XAI_API_KEY', area: 'AI provider', secret: true, requiredInProduction: false, note: 'Serves grok-* models (ADM-85). Unset ⇒ those models are not served.' },
  { key: 'XAI_BASE_URL', area: 'AI provider', secret: false, requiredInProduction: false, note: 'Test-only override; production must NOT point it at an external host.' },
